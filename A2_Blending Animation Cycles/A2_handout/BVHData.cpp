...
void BVHData::RenderJoint(Matrix4& viewMatrix, Matrix4 parentMatrix, Joint* joint, float scale, int frame)
	...
	// YOUR CODE GOES HERE
	// joint translation
	Cartesian3 translation(joint->joint_offset[0] * scale,joint->joint_offset[1] * scale,joint->joint_offset[2] * scale);
	// joint translation Matrix
	Matrix4 jointTranslationMatrix = Matrix4::Translate(translation);
	// joint rotation (depends on frames)
	Cartesian3 rotation = boneRotations[frame][joint->id];
	// joint rotation Matrix
	Matrix4 jointRotationMatrixX = Matrix4::RotateX(-rotation.x);//why is negative?
	Matrix4 jointRotationMatrixY = Matrix4::RotateY(-rotation.y);
	Matrix4 jointRotationMatrixZ = Matrix4::RotateZ(-rotation.z);
	Matrix4 jointRotationMatrix = jointRotationMatrixX * jointRotationMatrixY * jointRotationMatrixZ;
	// Convert joint positions to the world coordinate system
	Matrix4 jointMatrix = parentMatrix * jointTranslationMatrix* jointRotationMatrix; 
	//culculate word position
	Cartesian3 parentPosition = Cartesian3(parentMatrix[0][3],parentMatrix[1][3],parentMatrix[2][3]);
	Cartesian3 jointPosition = Cartesian3(jointMatrix[0][3],jointMatrix[1][3],jointMatrix[2][3]);
	//Cartesian3 parentWorldPosition = viewMatrix * parentPosition;
	//Cartesian3 jointWorldPosition = viewMatrix * jointPosition;
	//debug
	//std::cout<<"("<<jointPosition.x<< ", "<<jointPosition.y<<", "<<jointPosition.z<<")"<<std::endl;

	//RenderCylinder(viewMatrix,parentWorldPosition,jointWorldPosition);
	if(joint->id != 0){
	   RenderCylinder(viewMatrix,parentPosition,jointPosition);
	}	
	// Recursive rendering of sub joints
	for (size_t i = 0; i < joint->Children.size(); i++)
	    {	    
	    // Recursive rendering of sub joints
	    RenderJoint(viewMatrix, jointMatrix, &joint->Children[i], scale, frame);
	    }
	} // RenderJoint()
...
void BVHData::RenderCylinder(Matrix4& viewMatrix, Cartesian3 start, Cartesian3 end)
	...
	// direction
	Cartesian3 direction = end - start;
	float length = direction.length(); // length of cylinder
	// translation
	Matrix4 translationMatrix = Matrix4::Translate(start);
	// rotation	
	Matrix4 rotationMatrix = Matrix4::GetRotation(Cartesian3(0, 0, 2.0),direction);
	// model Matrix
	Matrix4 modelMatrix = Matrix4::RotateX(-90.0) * translationMatrix * rotationMatrix;
	// new view Matrix
	Matrix4 cylinderViewMatrix = viewMatrix * modelMatrix;//
	// render a single cylinder
	Cylinder(cylinderViewMatrix, 0.1, length, 16);
