...
const float initialLaunchSpeed = 5.0;
...
SceneModel::SceneModel()
	...
	// define the speed, position of ball
	ballPosition = Cartesian3(0.0, 0.0, 10.0);
	
	// set initial launch velocity components
	float launchVelocityX = initialLaunchSpeed * cos(launchAngle * PI / 180.0);
	float launchVelocityZ = initialLaunchSpeed * sin(launchAngle * PI / 180.0);
	ballSpeed = Cartesian3(launchVelocityX, 0.0, launchVelocityZ);

	frameTime = 0.04167f;// 1s/24frame
	gravity = 9.8f;
	elasticity = 0.6;
...
void SceneModel::Update()
	...
	if (useSphere)
		...
		// YOUR CODE GOES HERE
		  //count speed and position
		  ballPosition = ballPosition + (ballSpeed * frameTime);
		  ballSpeed.z = ballSpeed.z + (-gravity * frameTime);
		  //get land height
		  float landHeight = activeLandModel->getHeight(ballPosition.x,ballPosition.y);
		  //get land normal
		  Cartesian3 landNormal = activeLandModel->getNormal(ballPosition.x,ballPosition.y);
		  //std::cout << landHeight << std::endl;0
		  //std::cout << ballPosition.z << std::endl;
		  // Judging the contact between the ball and the ground
		  if(ballPosition.z-sphereRadius < landHeight)
		    {
		      // Adjust the position of the ball to the position where it has just made contact with the ground
		      ballPosition.z = sphereRadius+landHeight;
		      // rebound ball
		      ballSpeed = ballSpeed - 2.0f * landNormal * ballSpeed.dot(landNormal);
		      ballSpeed = ballSpeed * elasticity;

		      std::cout<<ballSpeed<<std::endl;
		      /*
		      if(ballSpeed.length() <= MinSpeed)
			{
			  ballSpeed = Cartesian3(0,0,0);
			}
		      */
		    }
		  // debug
