...
// and a routine to return the inertial tensor
// assuming all vertices are equal weight
Matrix3 IndexedFaceSurface::InertialTensor()
	{ // InertialTensor()
	// start off with a zero matrix
	Matrix3 theTensor = Matrix3::Zero();
	
	// loop through all of the vertices, adding their contributions to each entry
	for (int vertex = 0; vertex < (int) vertices.size(); vertex++)
		{ // per vertex
		// retrieve a copy of the vertex
		Cartesian3 theVertex = vertices[vertex];
		// add contributions to each element in turn 
		theTensor[0][0] += theVertex.y * theVertex.y + theVertex.z * theVertex.z;
		theTensor[0][1] -= theVertex.x * theVertex.y; 
		theTensor[0][2] -= theVertex.x * theVertex.z; 

		theTensor[1][0] -= theVertex.x * theVertex.y; 
		theTensor[1][1] += theVertex.x * theVertex.x + theVertex.y * theVertex.y;
		theTensor[1][2] -= theVertex.y * theVertex.z; 

		theTensor[2][0] -= theVertex.x * theVertex.z; 
		theTensor[2][1] -= theVertex.y * theVertex.z; 
		theTensor[2][2] += theVertex.x * theVertex.x + theVertex.y * theVertex.y;
		} // per vertex
